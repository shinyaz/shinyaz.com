...
- AWS CLI 2.34.16以上（`--filesystem-configurations`パラメータのサポートが必要）
- boto3 1.42.76以上（検証スクリプトで使用）
- 検証リージョン: us-west-2
...
`--filesystem-configurations`でセッションストレージのマウントパスを指定する。このパラメータはAWS CLI 2.34.16以降で利用可能だ。
