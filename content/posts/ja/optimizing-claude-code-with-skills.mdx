...
本記事では `.claude/skills/` に配置するファイルを一括して「スキル」と呼んでいるが、Claude Codeには**スラッシュコマンド**（`.claude/commands/`）と**スキル**（`.claude/skills/`）という2つの仕組みがあり、設計思想が異なる。『[実践Claude Code入門](https://gihyo.jp/book/2026/978-4-297-14698-8)』（技術評論社）では、この区別を以下のように整理している。
