...
[実践第 3 回](/ja/blog/2026/04/03/strands-agents-hooks)で学んだ Hooks は、マルチエージェントシステムにも使える。`BeforeNodeCallEvent` と `AfterNodeCallEvent` で Graph のノード実行を監視できる。以下のコードは前のセクションで作った `graph` に Hooks を追加する例だ（全体コードは折りたたみ内の `02_hooks.py` を参照）。
