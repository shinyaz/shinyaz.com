...
[第 1 回](/ja/blog/2026/04/11/strands-agents-swarm)で Swarm の自律協調を、[第 2 回](/ja/blog/2026/04/11/strands-agents-graph)で Graph の構造化ワークフローを学んだ。しかし、実際のタスクでは「リサーチは複数の専門家が自律的に協調し、全体のフローは構造化したい」のように、両方を組み合わせたい場面がある。
...
[第 1 回](/ja/blog/2026/04/11/strands-agents-swarm)の環境をそのまま使う。以降の例ではすべて同じモデル設定を使う。各例は独立した `.py` ファイルとして実行できる。共通設定を先頭に書き、その下に各例のコードを追加する形だ。
