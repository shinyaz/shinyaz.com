...
- [第1回](/ja/blog/2026/03/23/agentcore-cli-create-dev-deploy) — Runtime（基本ライフサイクル）
- [第2回](/ja/blog/2026/03/23/agentcore-cli-memory-persistence) — Memory（会話の記憶）
- [第3回](/ja/blog/2026/03/23/agentcore-cli-gateway-mcp-server) — Gateway（外部 MCP サーバー接続）
- [第4回](/ja/blog/2026/03/23/agentcore-cli-evaluations-quality) — Evaluations（品質計測）
...
- **us-east-1 リージョン**の AWS アカウント — Evaluator の CloudFormation リソースが ap-northeast-1 では未サポートのため（[第4回](/ja/blog/2026/03/23/agentcore-cli-evaluations-quality)参照）
...
[第3回](/ja/blog/2026/03/23/agentcore-cli-gateway-mcp-server)で学んだ通り、Gateway を使う場合は `--no-agent` でプロジェクトを作成し、Gateway 設定後にエージェントを追加する。
...
`main.py` の全体は、[第2回](/ja/blog/2026/03/23/agentcore-cli-memory-persistence)の Memory 統合コードと[第3回](/ja/blog/2026/03/23/agentcore-cli-gateway-mcp-server)の Gateway クライアントコードが組み合わさった構成だ。`get_all_gateway_mcp_clients()` で Gateway ツールを取得し、`get_memory_session_manager()` でセッションマネージャーを初期化する。
...
Memory の長期メモリ抽出を待つ（[第2回](/ja/blog/2026/03/23/agentcore-cli-memory-persistence)で確認した通り、1〜2分かかる）。
