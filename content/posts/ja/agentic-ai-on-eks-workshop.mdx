...
AWS が公開した[Agentic AI on EKS](https://catalog.workshops.aws/agentic-ai-on-eks/en-US)ワークショップは、この問いに対する一つの回答を示している。Strands Agents SDK、MCP（Model Context Protocol）、A2A（Agent-to-Agent）プロトコルを組み合わせ、EKS 上に本番対応のエージェントプラットフォームを構築するハンズオンだ。ソースコードは [aws-samples/sample-agentic-frameworks-on-aws](https://github.com/aws-samples/sample-agentic-frameworks-on-aws) リポジトリの `eks/` ディレクトリにある。
...
- `kubectl` と `aws` CLI が設定済み
- Bedrock の Claude Haiku 4.5 モデルアクセスが有効（cross-region inference profile `global.anthropic.claude-haiku-4-5-20251001-v1:0`）
...
本記事では各リソースの役割を理解するために手動で構築した。結果だけ見たい場合は[デプロイと動作確認](#デプロイと動作確認)まで読み飛ばしてほしい。

<details className="my-4 rounded-lg border border-border bg-muted/30 p-4">
<summary className="cursor-pointer font-medium">事前準備（リポジトリクローン、ECR、S3、Pod Identity）</summary>
...
export CLUSTER_NAME=eks-sandbox  # 自分のクラスター名に変更
```

ECR リポジトリの作成。4 コンポーネント分を一括で作る。
...
S3 セッションバケットの作成。Weather Agent と Travel Agent がそれぞれユーザーごとの会話履歴を保持する。
...
Pod Identity 用の IAM ロール作成。エージェントが Bedrock と S3 にアクセスするために必要だ。
...
Travel Agent 用も同様に作成する（S3 バケット名を `travel-agent-session-${ACCOUNT_ID}` に変更）。
...
<details className="my-4 rounded-lg border border-border bg-muted/30 p-4">
<summary className="cursor-pointer font-medium">Kaniko ビルド環境の構築手順</summary>
...
ビルドコンテキストを S3 にアップロードし、Kaniko Job を実行する。
...
ポイントは Pod Identity で ECR push 権限を付与すること。`ecr:PutImage` と `ecr:CompleteLayerUpload` を含むポリシーを `build` namespace の `kaniko` サービスアカウントに紐づけた。ビルド時間は MCP Server が約 2 分、Weather Agent が約 3 分だった。
...
`mcp-remote.yaml` は MCP 接続先を stdio（ローカル）から HTTP（リモート MCP Server）に切り替える values override ファイルだ。`a2a.http_url` は Part 2 の A2A 連携で必要になる設定で、エージェントカードに記載される URL を Kubernetes Service の FQDN に設定する。
...
動作確認では、NYC の 3 日間天気予報を正常に取得できた。クラスター内部から curl で直接アクセスするか、`kubectl port-forward` でローカルに転送して確認する。
