...
[前回の記事](/ja/blog/2026/03/24/aurora-blue-green-downtime-measurement)では、Aurora PostgreSQL の Blue/Green Switchover 中に psql で26秒・6回の接続失敗を観測した。DNS TTL 60秒がダウンタイムの主要因であることも確認した。
...
| HikariCP | 6.2.1 |
| クエリ | `SELECT inet_server_addr()` を1秒間隔で400回 |

前提条件:

- 前回の記事と同じ Aurora クラスター構成（論理レプリケーション有効化済み）
- Java 21 + Maven
- AWS CLI セットアップ済み
...
第1回と同じ Aurora PostgreSQL 16.9 クラスター（論理レプリケーション有効化済み）を使用する。クラスター構築手順は[前回の記事](/ja/blog/2026/03/24/aurora-blue-green-downtime-measurement)を参照。
...
Blue/Green デプロイメントの作成手順は[前回の記事](/ja/blog/2026/03/24/aurora-blue-green-downtime-measurement)の Step 3 と同じだ。Green 環境が `AVAILABLE` になったら、3パターンを同時にバックグラウンドで起動し、Switchover を実行する。
...
- **HikariCP のリトライだけでは不十分** — リトライで接続失敗は減るが、コネクションプールが旧 Writer への接続を保持し続けるため、Switchover 後も旧環境に接続し続ける。書き込みワークロードでは `read-only transaction` エラーが発生する。これは DNS ベースのエンドポイント切り替えとコネクションプールの根本的な相性の悪さだ。
