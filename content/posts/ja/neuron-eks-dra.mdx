...
```bash title="Terminal（EKS クラスター作成）"
...
```text title="Output（存在しないバージョン指定時）"
...
```bash title="Terminal（Helm と Kubernetes リソースの削除）"
# DRA ドライバーのアンインストール
...
```bash title="Terminal（EKS クラスターの削除）"
# ノードグループ → クラスターの順に削除
