...
[前回の記事](/ja/blog/2026/04/12/strands-agents-docker)では FastAPI + Docker でエージェントをコンテナ化した。コンテナは汎用的だが、常時起動のインフラが必要だ。

Lambda なら、リクエストが来たときだけ起動し、使った分だけ課金される。公式 Lambda Layer を使えば、ハンドラー関数を書くだけでサーバーレスエージェントが動く。

ただし、Lambda ではレスポンスのストリーミングに対応していない点に注意が必要だ。ストリーミングが必要な場合は[前回](/ja/blog/2026/04/12/strands-agents-docker)の Docker + Fargate 方式を検討してほしい。
...
- [第 1 回](/ja/blog/2026/04/12/strands-agents-docker)の環境（`strands-agents` インストール済み）をローカルテストに使用
...
[前回](/ja/blog/2026/04/12/strands-agents-docker)の FastAPI では `def invoke(request)` でリクエストを受け取った。Lambda では `def handler(event, context)` がエントリポイントになる。以下のハンドラーは `aws lambda invoke` と後述の Function URL の両方に対応している。
...
- **`Agent` はハンドラー内で作成する** — [前回](/ja/blog/2026/04/12/strands-agents-docker)と同じ理由で、リクエスト（invoke）ごとに新しいインスタンスを作成する
...
ここまでの Lambda 関数は `aws lambda invoke` でしか呼び出せない。[第 1 回](/ja/blog/2026/04/12/strands-agents-docker)の FastAPI のように HTTP で呼び出すには、[Lambda Function URL](https://docs.aws.amazon.com/lambda/latest/dg/urls-configuration.html) を追加する。API Gateway を使わずに、Lambda 関数に専用の HTTPS エンドポイントを付与できる。
...
[第 1 回](/ja/blog/2026/04/12/strands-agents-docker)の FastAPI と同じように、HTTP リクエストでエージェントを呼び出せるようになった。
...
- **`BedrockModel` はモジュールレベル、`Agent` はハンドラー内** — ウォーム起動でモデルを再利用しつつ、リクエストごとに会話を分離する。[前回](/ja/blog/2026/04/12/strands-agents-docker)の FastAPI と同じ設計原則。
