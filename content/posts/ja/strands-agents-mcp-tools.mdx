...
[前回の記事](/ja/blog/2026/03/24/strands-agents-custom-tools)ではカスタムツールの設計パターンを学んだ。`@tool` デコレータで自作ツールを作れるようになったが、すべてのツールを自分で実装するのは現実的ではない。
...
[前回の記事](/ja/blog/2026/03/24/strands-agents-custom-tools)の環境をそのまま使う。追加で [uv](https://github.com/astral-sh/uv) が必要だ（MCP サーバーの実行に `uvx` コマンドを使う）。
