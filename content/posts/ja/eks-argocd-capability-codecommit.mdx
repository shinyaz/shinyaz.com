...
description: "Capability Role に IAM ポリシーを1つ追加するだけで CodeCommit プライベートリポジトリからデプロイできる。URL typo と権限不足が同じエラーになる点と、権限修正後に手動 refresh が必要な点が落とし穴。"
date: "2026-04-03"
published: true
featured: false
...
EKS ArgoCD Capability は CodeCommit との「直接統合」をサポートしている。Capability Role に `codecommit:GitPull` を付与するだけで、Repository Secret を作成せずにプライベートリポジトリからデプロイできる。セルフホスト ArgoCD では Git 認証情報を Kubernetes Secret として管理する必要があったが、Capability 版では IAM に統合されている。
...
Capability Role に `codecommit:GitPull` のインラインポリシーを追加する。
...
Application を作成する。`repoURL` に CodeCommit の HTTPS URL を直接指定する。Repository Secret は作成しない。
...
ポーリング間隔（約3分）+ IAM 認証のオーバーヘッドを考慮すると、5分程度は想定内だ。より即時性が必要な場合は [Webhook の設定](https://docs.aws.amazon.com/eks/latest/userguide/argocd-considerations.html) を検討するとよい。

## 検証 2: 設定ミス時のエラー挙動とデバッグ方法

直接統合では ArgoCD UI の Settings → Repositories にリポジトリが表示されない。Repository Secret を作成していないため、ArgoCD はリポジトリの存在を「知らない」状態だ。接続テストも UI からは行えない。
...
ただし、**権限を付与した後に Application が自動的にリカバリしない**点に注意が必要だ。IAM ポリシーを追加しても、ArgoCD の次のポーリングサイクルでは同じエラーが継続した。手動で hard refresh を実行する必要があった。
...
存在しないリポジトリ名（`argocd-demo-typo`）を指定した場合のエラーを確認した。
...
**URL typo でも権限不足でも、同じ `AccessDeniedException` が返る。** IAM ポリシーが特定のリポジトリ ARN にスコープされているため、存在しないリポジトリへのアクセスも「権限がない」として処理される。「リポジトリが見つからない」というエラーにはならない。
...
Repository Secret を作成すると、ArgoCD UI の Settings → Repositories にリポジトリが表示され、接続ステータスを確認できるようになる。
...
- **URL typo と権限不足が同じエラーになる** — IAM ポリシーがリソース ARN にスコープされているため、存在しないリポジトリへのアクセスも `AccessDeniedException` として返る。エラーが出たら、まず IAM ポリシーの `Resource` と `repoURL` のリポジトリ名を突き合わせるのが最初のデバッグステップだ
- **権限修正後は手動 refresh が必要** — IAM ポリシーを追加しても、ArgoCD は自動的にリカバリしない。`argocd.argoproj.io/refresh=hard` アノテーションで明示的に refresh を実行する必要がある
