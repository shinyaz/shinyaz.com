...
`StorageEncrypted: false` は一見すると暗号化が無効に見えるが、公式ドキュメントによると Express Configuration のクラスターは AWS/RDS サービス所有キーで暗号化されている。`StorageEncrypted` はカスタマーマネージド KMS キーの使用有無を示すフィールドであり、`sse-rds`（RDS サービス所有キー）による暗号化は別扱いだ。カスタマーマネージドキーへの変更はできない。
