...
1. **2つの障害期間が発生した** — 最初の約10秒間は接続拒否（即座に失敗）、その後正常に戻ったが、約36秒後に再び3秒のタイムアウトが連続で発生した。これは DNS キャッシュの TTL（60秒）が切れるタイミングで、新しい DNS 解決結果が旧 Writer の IP を返し、その旧 Writer がすでにシャットダウン中だったためタイムアウトしたと考えられる。
