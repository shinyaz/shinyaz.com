...
description: "Aurora PostgreSQL のフェイルオーバーと Blue/Green Switchover のダウンタイムを実測比較。フェイルオーバーは約10秒、Blue/Green Switchover は約26秒だが PG16→17 のメジャーアップグレードを含む。DNS TTL 60秒の影響も検証した。"
date: "2026-03-24"
published: true
featured: true
...
セットアップから Switchover まで全工程を再現可能な形で記載する。結果だけ知りたい場合は [検証結果まとめ](#検証結果まとめ) にスキップできる。
...
クラスターとインスタンスの作成には約10〜15分かかった。両方のインスタンスが `available` になったら接続を確認する。
...
10:02:24.839  #61  OK    134ms  172.31.37.206  ← 復旧
```
...
| --- | --- |
| 接続失敗回数 | 10回 |
| 第1障害期間 | 約10秒（10:01:30〜10:01:40） |
| 第2障害期間 | 約4秒（10:02:16〜10:02:20、タイムアウト含む） |
| 障害パターン | 2つの障害期間に分離 |

注目すべき点が2つある。

1. **2つの障害期間が発生した** — 最初の約10秒間は接続拒否（即座に失敗）、その後正常に戻ったが、約40秒後に再び3秒のタイムアウトが2回発生した。これは DNS キャッシュの TTL（60秒）が切れるタイミングで、古い IP への接続がタイムアウトしたと考えられる。
2. **復旧後も同じ IP に接続している** — フェイルオーバー後、クラスターエンドポイントの CNAME は新しい Writer（旧 Reader）を指すように更新されるが、DNS キャッシュが残っている間は古い IP に接続し続ける。今回は旧 Writer がまだ起動中だったため接続自体は成功したが、書き込みを行うワークロードであればエラーになっていた可能性がある。

## Step 3: Blue/Green デプロイメントの構築と Switchover

### 論理レプリケーションの有効化

Blue/Green デプロイメントには論理レプリケーションが必要だ。カスタムパラメータグループを作成してクラスターに適用する。
...
パラメータグループの適用後、**全インスタンスの再起動が必要**だ。`pending-reboot` ステータスが `in-sync` になるまで待つ。
...
Green 環境のプロビジョニングは3つのフェーズで進行する。
...
1. **失敗はすべてタイムアウト** — フェイルオーバーでは即座に接続拒否されるケースがあったが、Switchover では全て3秒のタイムアウト。これは Switchover 中に Blue 環境が書き込みを停止し、接続を受け付けなくなるためだ。
2. **IP が明確に切り替わる** — フェイルオーバーでは同じ IP に接続し続けたが、Switchover では Green 環境の新しい IP に切り替わった。
...
- **Blue/Green Switchover のダウンタイムは「長いが予測可能」** — フェイルオーバーの10秒に対して Switchover は26秒と長いが、失敗パターンが均一（すべてタイムアウト）で予測しやすい。リトライロジックの設計がしやすい。
