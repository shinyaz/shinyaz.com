...
<summary className="cursor-pointer font-medium">IAM ロール・Agent Space 作成手順</summary>

2つの IAM ロールが必要である。Agent Space 用ロール（リソース検出・調査用）と Operator App 用ロール（Web App アクセス用）だ。

```bash title="Terminal (Agent Space ロール作成)"
...
```bash title="Terminal (Operator App ロール作成)"
...
```bash title="Terminal (Agent Space 作成・アカウント関連付け・Operator App 有効化)"
...
> インスタンス i-09329356f21adb8a5 で 2026-04-01 00:42:09 UTC に Systems Manager 経由で `stress-ng --cpu 2 --timeout 600` コマンドが実行された。t3.micro の2 vCPU を stress-ng が完全に使用していることが、100% の CPU 使用率の直接的な原因である。
