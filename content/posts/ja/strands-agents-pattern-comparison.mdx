...
[第 1 回](/ja/blog/2026/04/11/strands-agents-swarm)の環境をそのまま使う。以降の例ではすべて同じモデル設定を使う。共通設定を先頭に書き、その下に各例のコードを追加する形だ。
...
[第 1 回](/ja/blog/2026/04/11/strands-agents-swarm)のパターン。エージェント同士が自律的にハンドオフする。
...
[第 2 回](/ja/blog/2026/04/11/strands-agents-graph)のパターン。エッジで実行順序を明示的に定義する。
...
| [マルチエージェント](/ja/blog/2026/04/11/strands-agents-swarm)（全 4 回） | 協調パターン | Swarm、Graph、ネスト+Hooks+共有状態、パターン比較 |
