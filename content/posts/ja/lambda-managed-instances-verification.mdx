---
title: "Lambda Managed Instances を検証 — プロビジョニング時間・マルチコンカレンシー・スケーリングの実態"
...
2026年3月30日、AWS は [Lambda Managed Instances (LMI)](https://aws.amazon.com/jp/blogs/compute/build-high-performance-apps-with-aws-lambda-managed-instances/) を発表した。Lambda のプログラミングモデルを維持しながら、EC2 インスタンス上で関数を実行できる新しいコンピュートオプションである。
...
本記事では LMI を実際にデプロイし、3つの核心的な挙動を計測する。結果をもとに「自分のワークロードは LMI に移行すべきか」を判断するためのチェックリストを提示する。公式ドキュメントは [Lambda Managed Instances](https://docs.aws.amazon.com/lambda/latest/dg/lambda-managed-instances.html) を参照。
...
バージョンが Active になった後の invoke では、コールドスタートは発生しなかった。5回の invoke で安定して約1.2秒（ネットワーク往復含む）。
...
PID がすべて異なる点に注目してほしい。5回の逐次 invoke が5つの異なる実行環境にルーティングされている。MinExecutionEnvironments=3 の設定だが、実際にはより多くの実行環境が起動していた（後述）。
...
MinExecutionEnvironments=3、PerExecutionEnvironmentMaxConcurrency=10 なので、理論上限は 3 × 10 = 30 と予想した。
...
**30 並列で全て成功、スロットリングなし。** しかし PID が 10 個確認された。MinExecutionEnvironments=3 にもかかわらず、Lambda は 10 の実行環境を起動していた。各実行環境に3リクエストずつ均等に分散されている。
...
40 並列で 6 件のスロットリングが発生した。完了時間も 9.2 秒に伸びている。10 実行環境 × MaxConcurrency 10 = 100 が理論上限のはずだが、I/O バウンドでも 40 並列で一部スロットリングが発生した。リクエストのルーティングやキューイングのオーバーヘッドが影響していると考えられる。

### 所感

**マルチコンカレンシーは I/O バウンドワークロードで効果的に機能する。** 3秒の sleep を含む処理が、10 並列でも約4.5秒で完了する。標準 Lambda なら10の実行環境が必要な処理を、より少ないリソースで処理できる。

一方で、MinExecutionEnvironments=3 の設定に対して実際には 10 の実行環境が起動していた点は注意が必要である。Lambda が自動的にスケールアウトしているため、最小コストの見積もりは MinExecutionEnvironments だけでは判断できない。

## 検証 3: CPU 負荷でスケールアウトはどれだけ速いか

LMI のスケーリングは CPU 使用率ベースで非同期に行われる。標準 Lambda のリクエスト駆動スケーリング（即時）とどう違うかを、CPU バウンドワークロードで検証した。
...
**CPU バウンドでは大量のスロットリングが発生し、30秒の間隔を挟んでもスケールアウトが観察されなかった。** PID 数は 10 のまま変化していない。

MaxExecutionEnvironments を明示的に 20 に設定して再テストしたところ、PID 25 が新たに出現（11 ユニーク PID）。スケールアウトは始まったが、即座ではなかった。

### 30 並列 × 5 バッチ（15秒間隔）での安定性

並列数を 30 に下げて継続的に負荷をかけた結果:
...
10 の実行環境で 30 並列を安定処理でき、バッチごとに処理時間が短縮（18秒→12秒）している。実行環境のウォームアップ効果と考えられる。
...
CPU バウンドの場合、マルチコンカレンシーの恩恵も限定的である。CPU リソースを奪い合うため、並列数を増やしても個々のリクエストの処理時間が伸びる。PerExecutionEnvironmentMaxConcurrency を vCPU 数以下に設定するか、vCPU 数を増やすことを検討すべきである。
...
| スケールアウト速度 | リクエスト単位で即時 | CPU ベースで数分 |

### コスト構造の違い

LMI は EC2 インスタンス課金 + 15% 管理手数料。今回の検証では m7i.xlarge（$0.2016/時間 × 3台）が起動した。

- LMI の最低コスト: 3台 × $0.2016 × 1.15 ≈ **$0.70/時間**（MinExecutionEnvironments=3 でも3台は常時稼働）
...
| チェック項目 | LMI 向き | 標準 Lambda 向き |
| --- | --- | --- |
| トラフィックパターン | 定常的・予測可能 | バースト・スパイク型 |
| コールドスタート許容度 | 許容できない | 許容できる |
| 実行時間 | 長時間・定常処理 | 短時間・イベント駆動 |
| ワークロード特性 | I/O バウンド（マルチコンカレンシーの恩恵大） | CPU バウンド（スケーリングが課題） |
| 稼働率 | 高稼働（EC2 課金が有利） | 低稼働（リクエスト課金が有利） |
| VPC 要件 | すでに VPC 内で運用 | VPC 不要 |
| デプロイ頻度 | 低頻度（67秒の待ちを許容） | 高頻度（即時デプロイが必要） |
...
<details className="my-4 rounded-lg border border-border bg-muted/30 p-4">
<summary className="cursor-pointer font-medium">クリーンアップ（リソース削除コマンド）</summary>
