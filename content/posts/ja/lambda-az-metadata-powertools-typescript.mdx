...
[前回の記事](/ja/posts/lambda-az-metadata-same-az-routing)では、Lambda AZメタデータエンドポイントを直接呼び出してSame-AZルーティングの効果を検証した。結果としてクロスAZレイテンシが同一AZの約2.5倍であることを実測で確認できたが、直接APIアクセスではHTTPリクエスト構築、認証トークン処理、キャッシュ実装をすべて自前で行う必要があった。
...
- [前回の記事](/ja/posts/lambda-az-metadata-same-az-routing)の検証環境と同等の構成（ElastiCache Valkey 3AZ、VPC内Lambda）
- Node.js 22.x ランタイム
...
[前回の記事](/ja/posts/lambda-az-metadata-same-az-routing)と同条件（ElastiCache Valkey 3AZ、TLS、PING 50回）で、TypeScript Powertools版のSame-AZルーティングを検証した。
...
ElastiCache Valkey クラスターは[前回の記事](/ja/posts/lambda-az-metadata-same-az-routing)のデプロイ手順で構築する。TypeScript Lambda のビルドとデプロイは以下の通り。
...
`CACHE_NODES` の形式は[前回の記事](/ja/posts/lambda-az-metadata-same-az-routing)を参照。
...
[前回の記事](/ja/posts/lambda-az-metadata-same-az-routing)と同じ手順でリソースを削除する。TypeScript版のLambda関数名のみ異なる。
