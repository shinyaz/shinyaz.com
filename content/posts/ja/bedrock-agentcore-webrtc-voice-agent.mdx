...
```bash title="Terminal（ブラウザクライアント起動）"
cd server
...
`agentcore destroy` 後、AgentCore Runtime が VPC 内に作成した ENI の解放に時間がかかる場合がある（数分〜数時間）。サブネットや VPC の削除で `DependencyViolation` エラーが出た場合は、ENI が `available` になるまで待ってから再試行する。
