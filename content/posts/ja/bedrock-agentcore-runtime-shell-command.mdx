...
各コマンドは新しいbashプロセスとして実行される「ワンショット実行」モデルだ。シェルヒストリーや環境変数はコマンド間で引き継がれない。ただし、後述の検証で確認した通り、ファイルシステムは同一セッション内で共有されるため、シェル状態の引き継ぎが必要な場合のみ`&&`チェーンを使えばよい。
...
ドキュメントでは「各コマンドは新しいbashプロセス」と記載されているが、**ファイルシステムは同一セッション内で共有される。** これはmicroVM単位でセッションが管理されているためで、`&&`チェーンを使わなくても中間成果物をファイル経由で受け渡せる。
...
- **「ステートレス」の実態はシェル状態のみ** — ドキュメントには「Stateless between commands」とあるが、実際にはファイルシステムは同一セッション内で永続化される。引き継がれないのはシェルヒストリーや環境変数などのシェルプロセス状態だ。
