...
[前回までの記事](/ja/blog/2026/03/24/strands-agents-mcp-tools)では、カスタムツールや MCP でエージェントの「できること」を拡張してきた。しかし、これまでの例はすべて 1 回きりの呼び出しだった。実用的なエージェントには「会話を続ける」能力が必要だ。
...
[前回の記事](/ja/blog/2026/03/24/strands-agents-mcp-tools)の環境をそのまま使う。
