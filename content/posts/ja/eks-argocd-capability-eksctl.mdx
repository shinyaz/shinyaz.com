...
[前回の記事](/ja/posts/eks-argocd-capability)では、AWS CLI を使って EKS ArgoCD Capability を構築した。CLI は細かい制御が効くが、JSON のインラインパラメータが長くなりがちだ。
...
- EKS クラスター（Auto Mode）が稼働中であること（[クラスター構築手順](/ja/posts/eks-auto-mode-cluster-setup)を参照）
...
[前回の記事](/ja/posts/eks-argocd-capability)と同じ手順で、ArgoCD Capability 用の [IAM ロール](https://docs.aws.amazon.com/eks/latest/userguide/capability-role.html)を作成する。
...
Identity Center のインスタンス ARN、Identity Store ID、ユーザー ID を取得する。Identity Center のホームリージョンの特定方法は[前回の記事](/ja/posts/eks-argocd-capability)を参照。
...
[前回の記事](/ja/posts/eks-argocd-capability)と同様に、アクセスポリシーの関連付けとクラスター登録を行う。
