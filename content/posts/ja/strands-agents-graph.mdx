...
[前回の記事](/ja/blog/2026/04/11/strands-agents-swarm)では Swarm でエージェントが自律的にハンドオフする仕組みを学んだ。しかし、Swarm はエージェントが自律的に判断するため、実行順序が予測しにくい。「リサーチ → 分析 → レポート」のように実行順序を明示的に制御したい場面がある。
