...
[第1回](/ja/blog/2026/03/23/agentcore-cli-create-dev-deploy)では基本ライフサイクル、[第2回](/ja/blog/2026/03/23/agentcore-cli-memory-persistence)では Memory による記憶の永続化を検証した。今回は Gateway を使って、外部の MCP サーバーをエージェントに接続する。
...
`main.py` では、[第1回](/ja/blog/2026/03/23/agentcore-cli-create-dev-deploy)のデフォルトテンプレートの `get_streamable_http_mcp_client()` の代わりに `get_all_gateway_mcp_clients()` が使われる。
