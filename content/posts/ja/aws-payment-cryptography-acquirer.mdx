...
アクワイアラは端末から受け取った決済データを、決済ネットワークやイシュアに中継する役割を担う。その過程で、端末の鍵で暗号化された PIN をネットワークの鍵に変換したり、データの改ざんがないことを MAC で検証したりする。
...
<details className="my-4 rounded-lg border border-border bg-muted/30 p-4">
<summary className="cursor-pointer font-medium">GeneratePinData のコード（イシュア編と同じ）</summary>
...
<details className="my-4 rounded-lg border border-border bg-muted/30 p-4">
<summary className="cursor-pointer font-medium">VerifyPinData のコード</summary>
...
検証では、元のメッセージ `48656C6C6F576F726C64`（"HelloWorld"）の最後の 1 バイトを `64` → `65` に変更して改ざんをシミュレートした。

<details className="my-4 rounded-lg border border-border bg-muted/30 p-4">
<summary className="cursor-pointer font-medium">VerifyMac のコード（正常系・改ざん系）</summary>
...
// 改ざんメッセージで検証（最後の 1 バイトを 64→65 に変更）
dataPlane.verifyMac(VerifyMacRequest.builder()
        .keyIdentifier(macKeyArn)
        .messageData("48656C6C6F576F726C65")  // 末尾 64→65
        .mac(macResp.mac())
        .verificationAttributes(MacAttributes.builder()
                .algorithm(MacAlgorithm.CMAC).build())
        .build());
...
1 バイトの変更でも MAC 検証が失敗し、トランザクションデータの改ざんを検知できる。
