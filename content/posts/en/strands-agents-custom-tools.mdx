...
In the [previous article](/en/blog/2026/03/03/strands-agents-quickstart), we ran through the Strands Agents SDK Quickstart and confirmed how the agent loop works. We learned that the `@tool` decorator turns a function into a tool, but building practical agents requires understanding how to design tools well.
...
Use the same environment from the [previous article](/en/blog/2026/03/03/strands-agents-quickstart). For a fresh start:
