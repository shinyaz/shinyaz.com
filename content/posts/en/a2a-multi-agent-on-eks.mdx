...
In [Part 1](/en/blog/2026/03/23/agentic-ai-on-eks-workshop), I deployed a Weather Agent and MCP Server on EKS, validating the flow of tool auto-discovery and external API calls.
...
<summary className="cursor-pointer font-medium">Travel Agent deployment steps</summary>

Build the Travel Agent container image (same Kaniko workflow as Part 1) and deploy with Helm. The key is specifying the Weather Agent's A2A endpoint in `a2a.a2a_agents.json`.
