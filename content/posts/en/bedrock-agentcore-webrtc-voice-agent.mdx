...
After `agentcore destroy`, the ENI that AgentCore Runtime created in the VPC may take time to be released (minutes to hours). If you get a `DependencyViolation` error when deleting the subnet or VPC, wait until the ENI becomes `available` and retry.
