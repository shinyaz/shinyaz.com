...
- [Part 1](/en/blog/2026/03/23/agentcore-cli-create-dev-deploy) — Runtime (basic lifecycle)
- [Part 2](/en/blog/2026/03/23/agentcore-cli-memory-persistence) — Memory (conversation persistence)
- [Part 3](/en/blog/2026/03/23/agentcore-cli-gateway-mcp-server) — Gateway (external MCP server)
- [Part 4](/en/blog/2026/03/23/agentcore-cli-evaluations-quality) — Evaluations (quality measurement)
...
- **us-east-1 region** — Evaluator CloudFormation resource is not supported in ap-northeast-1 (see [Part 4](/en/blog/2026/03/23/agentcore-cli-evaluations-quality))
...
As learned in [Part 3](/en/blog/2026/03/23/agentcore-cli-gateway-mcp-server), Gateway requires `--no-agent` first, then adding the Gateway before the agent.
...
The full `main.py` combines the Memory integration from [Part 2](/en/blog/2026/03/23/agentcore-cli-memory-persistence) and the Gateway client from [Part 3](/en/blog/2026/03/23/agentcore-cli-gateway-mcp-server). It uses `get_all_gateway_mcp_clients()` for Gateway tools and `get_memory_session_manager()` for session management.
...
Wait for Memory's long-term extraction (1–2 minutes, as confirmed in [Part 2](/en/blog/2026/03/23/agentcore-cli-memory-persistence)).
