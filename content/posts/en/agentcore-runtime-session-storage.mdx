...
As of March 2026, `--filesystem-configurations` was not recognized by the installed AWS CLI version (`Unknown options` error). The official documentation includes a CLI example, so it may work with newer CLI versions. This article uses boto3.
...
```bash title="Before stop"
/mnt/workspace/project/README.md
/mnt/workspace/project/link-to-readme -> /mnt/workspace/project/README.md
/mnt/workspace/project/src/main.py  (permissions: 755)
```

After resuming (15-second wait):

```bash title="After resume"
/mnt/workspace/project/README.md          # ✅ Content matches
/mnt/workspace/project/link-to-readme     # ✅ Symlink restored
/mnt/workspace/project/src/main.py        # ✅ Permissions 755 preserved
```
...
# Verify restoration
r = run_command(
    'PYTHONPATH=/mnt/workspace/pylibs python3 -c "import requests; print(f\'requests {requests.__version__}\')" && '
    'cd /mnt/workspace/myproject && git log --oneline && git status && '
    'cat src/app.py && du -sh /mnt/workspace/*',
    session_id=session
)
print(r["stdout"])
```
