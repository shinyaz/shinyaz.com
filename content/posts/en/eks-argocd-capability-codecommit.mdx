...
description: "Just one IAM policy addition to the Capability Role enables CodeCommit private repo deployments. Watch out: URL typos and missing permissions produce the same error, and manual refresh is needed after fixing permissions."
date: "2026-04-03"
published: true
featured: false
...
EKS ArgoCD Capability supports "direct integration" with CodeCommit. By adding `codecommit:GitPull` to the Capability Role, you can deploy from a private repository without creating a Repository Secret. Unlike self-hosted ArgoCD, which requires managing Git credentials as Kubernetes Secrets, the Capability version handles authentication entirely through IAM.
...
Add a `codecommit:GitPull` inline policy to the Capability Role.
...
Create an Application with the CodeCommit HTTPS URL as `repoURL`. No Repository Secret is needed.
...
Considering the polling interval (~3 min) plus IAM authentication overhead, ~5 minutes is within expectations. For faster feedback, consider configuring [webhooks](https://docs.aws.amazon.com/eks/latest/userguide/argocd-considerations.html).

## Verification 2: Error Behavior and Debugging on Misconfiguration

With direct integration, the repository doesn't appear in ArgoCD UI's Settings → Repositories. Since no Repository Secret exists, ArgoCD doesn't "know" about the repository. Connection testing from the UI isn't possible either.
...
However, **the Application doesn't automatically recover after adding the permission**. Even after attaching the IAM policy, the same error persisted through ArgoCD's next polling cycle. A manual hard refresh was required:
...
I tested with a non-existent repository name (`argocd-demo-typo`):
...
**Both URL typos and missing permissions produce the same `AccessDeniedException`.** Because the IAM policy is scoped to a specific repository ARN, accessing a non-existent repository is also treated as "not authorized" rather than "not found."
...
With a Repository Secret, the repository appears in ArgoCD UI's Settings → Repositories, where you can check connection status.
...
- **URL typos and missing permissions produce the same error** — Because the IAM policy is scoped to a specific repository ARN, non-existent repositories also return `AccessDeniedException`. When debugging, always cross-check the IAM policy's `Resource` ARN against the Application's `repoURL` first
- **Manual refresh is needed after fixing permissions** — ArgoCD doesn't automatically recover from IAM errors. After adding the missing policy, run `argocd.argoproj.io/refresh=hard` to trigger an immediate retry
