...
1. **Two distinct outage windows** — The first ~10 seconds saw immediate connection refusals. After recovery, four consecutive 3-second timeouts occurred ~36 seconds later. This aligns with the DNS TTL (60s) expiring and the new DNS resolution returning the old writer's IP, which was already shutting down.
