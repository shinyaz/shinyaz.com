...
`StorageEncrypted: false` looks like encryption is disabled, but per the official documentation, Express Configuration clusters are encrypted with AWS/RDS service-owned keys. The `StorageEncrypted` field indicates whether a customer-managed KMS key is used, while `sse-rds` (RDS service-owned key) encryption is handled separately. You cannot switch to a customer-managed key.
