...
Create ECR repositories for all 4 components.
...
Create S3 session buckets. The Weather Agent and Travel Agent each store per-user conversation history.
...
Create the Travel Agent role similarly (change the S3 bucket name to `travel-agent-session-${ACCOUNT_ID}`).
