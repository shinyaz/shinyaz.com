...
In the [previous post](/en/blog/2026/03/21/eks-argocd-capability), I set up EKS ArgoCD Capability using AWS CLI. The CLI gives fine-grained control, but the inline JSON parameters get unwieldy.
...
- An EKS Auto Mode cluster up and running (see [cluster setup guide](/en/blog/2026/03/18/eks-auto-mode-cluster-setup))
...
Same as the [previous post](/en/blog/2026/03/21/eks-argocd-capability)—create a [dedicated IAM role](https://docs.aws.amazon.com/eks/latest/userguide/capability-role.html) with the Capabilities trust policy.
...
Retrieve the instance ARN, Identity Store ID, and user ID. See the [previous post](/en/blog/2026/03/21/eks-argocd-capability) for how to identify the Identity Center home region.
...
```yaml title="argocd-capability.yaml"
...
Same as the [previous post](/en/blog/2026/03/21/eks-argocd-capability)—associate an access policy and register the cluster.
