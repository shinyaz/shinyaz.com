...
Each command runs as a one-shot bash process — no shell history or environment variable persistence between calls. However, as verified below, the filesystem is shared within a session, so you only need `&&` chaining when shell state carryover is required.
...
Despite the "new bash process per command" model, **the filesystem is shared within a session** because the session maps to a single microVM. You can pass intermediate artifacts through files without `&&` chaining.
...
- **"Stateless" only applies to shell process state** — The docs say "Stateless between commands," but the filesystem actually persists within a session. Only shell history and environment variables are reset between commands.
