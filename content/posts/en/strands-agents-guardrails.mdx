...
In the [previous article](/en/blog/2026/04/03/strands-agents-hooks), we learned to control agent behavior with Hooks — limiting tool calls and modifying results. But there's another critical control: the safety of agent inputs and outputs themselves.
...
Before deploying guardrails to production, you may want to verify with a "shadow mode" that logs what would be blocked without actually blocking. We'll use Hooks from the [previous article](/en/blog/2026/04/03/strands-agents-hooks) to implement this.
