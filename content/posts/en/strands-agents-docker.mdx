...
`InvokeRequest` and `InvokeResponse` are Pydantic models that define the JSON structure for requests and responses. In [practical part 1](/en/blog/2026/03/29/strands-agents-structured-output), we used Pydantic to structure LLM output — here it's used for FastAPI input/output validation.
