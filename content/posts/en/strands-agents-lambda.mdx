...
In the [previous article](/en/blog/2026/04/19/strands-agents-docker), we containerized the agent with FastAPI + Docker. Containers are versatile but require always-on infrastructure.
...
Note that Lambda does not support response streaming. If you need streaming, consider the Docker + Fargate approach from the [previous article](/en/blog/2026/04/19/strands-agents-docker).
...
- The [part 1](/en/blog/2026/04/19/strands-agents-docker) environment (`strands-agents` installed) for local testing
...
In the [previous article](/en/blog/2026/04/19/strands-agents-docker), FastAPI used `def invoke(request)` to receive requests. In Lambda, `def handler(event, context)` is the entry point. The handler below supports both `aws lambda invoke` and Function URL (covered later).
...
- **`Agent` inside the handler** — Same reason as the [previous article](/en/blog/2026/04/19/strands-agents-docker): create a new instance per invocation to isolate conversation history
...
So far, the Lambda function can only be called via `aws lambda invoke`. To call it over HTTP like the FastAPI setup in [part 1](/en/blog/2026/04/19/strands-agents-docker), add a [Lambda Function URL](https://docs.aws.amazon.com/lambda/latest/dg/urls-configuration.html). This gives the function a dedicated HTTPS endpoint without needing API Gateway.
...
The agent is now callable over HTTP, just like the FastAPI setup in [part 1](/en/blog/2026/04/19/strands-agents-docker).
...
- **`BedrockModel` at module level, `Agent` in the handler** — Reuse the model across warm invocations while isolating conversations per request. Same design principle as the [previous article](/en/blog/2026/04/19/strands-agents-docker).
