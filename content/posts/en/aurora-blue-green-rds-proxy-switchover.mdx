...
| `<vpc-id>` | VPC ID |
| `<sg-id>` | Security group ID (created below) |
| `<subnet-1>`, `<subnet-2>`, `<subnet-3>` | Subnet IDs in different AZs (3 required) |
| `<subnet-id>` | Subnet ID for the EC2 instance |
| `<password>` | Database master password |
| `<secret-arn>` | Secrets Manager secret ARN |
| `<al2023-ami-id>` | Amazon Linux 2023 AMI ID |
| `<id>` | Identifier portion of the RDS Proxy endpoint |
